...
impl LLamaParams<f32> {
    pub fn from_safetensors(safetensor: &SafeTensors, config: &LlamaConfigJson) -> Self {
        ...
        let get_tensor= |name:&str| {
            let res = safetensor.tensor(name).unwrap();
            let size: usize = res.shape().iter().product();
            let data = unsafe { slice::from_raw_parts(res.data().as_ptr() as *const f32, size) };
            Tensor::new(data.to_vec(),&res.shape().to_vec())
        };
